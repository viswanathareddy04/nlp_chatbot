            ...
            if user_intent != 'goodbye':
                if user_response.lower().startswith('c0'):
                    ...
                    if botResponses.student_information.get(student_id, {}).get('name') == None:
                        return update.message.reply_text("Sorry I wasn't be able to find you in the database, make sure you have entered correct ID") 
                    resp = str(random.choice(botResponses.responses[1]['response'])).format( username=botResponses.student_information.get(student_id, {}).get('name'))
                ...
                if user_response == '/start' or not student_id:
                    resp = str(random.choice(
                        botResponses.responses[0]['response'])) + ", Before we start, can I get your student ID please? ex: C0X"
                    update.message.reply_text(resp)
                    return

                if user_intent == "grade":
                    resp = str(random.choice(botResponses.responses[11]['response'])).format(username=student_info.get('name'),
                                                                                grades=student_info.get('grades'))
                    update.message.reply_text(resp)
                    return

                if user_intent == "fee":
                    resp = str(random.choice(botResponses.responses[12]['response'])).format(username=student_info.get('name'),
                                                                                college_fee=student_info.get('fee'))
                    update.message.reply_text(resp)
                    return

                if user_intent == "date":
                    resp = str(random.choice(botResponses.responses[13]['response'])).format(username=student_info.get('name'),
                ...
                if user_intent == "time":
                    ...
                    if today == "Saturday" or today == "Sunday":
                        ...
                        class_subj = "You have" + botResponses.subjects.get(today) + "please use this URL to join class https://moodle.cestarcollege.com/moodle/mod/url/view.php?id=1003405"
                    
                    resp = str(random.choice(botResponses.responses[14]['response'])).format(username=student_info.get('name'),
