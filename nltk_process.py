...
tfidf = TfidfVectorizer(tokenizer=self.Normalize, stop_words='english')
...
class NLTKProcess:
    ...
    def performLogisticRegression( self, dataset ):
            ...
            self.lr.fit(X, y)
            X_test = ["exam"]
            prediction = self.lr.predict(self.vectorizer.transform(X_test))
            self.lr.predict_proba(self.vectorizer.transform(X_test))
    ...
    def response(self,user_response):
        ...
        X_test = self.vectorizer.transform(text_test)
    ...
    def intent(self,user_request):
        ...
        X_test_intent = self.vectorizer.transform(text_intent)
